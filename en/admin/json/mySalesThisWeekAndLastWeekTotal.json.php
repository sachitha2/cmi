...
$data['dealThis'] = $DB->nRow("deals","WHERE WEEK(fdate) =  WEEK(curdate()) AND MONTH(fdate)  = MONTH(curdate()) AND YEAR(fdate) = YEAR(curdate())  AND agentId = {$_SESSION['login']['userId']}");
$data['dealLast'] = $DB->nRow("deals"," WHERE YEAR(fdate) = YEAR(CURRENT_DATE) AND MONTH(fdate) = MONTH(CURRENT_DATE) AND WEEK(fdate) = WEEK(CURRENT_DATE - INTERVAL 1 WEEK)  AND agentId = {$_SESSION['login']['userId']}");
