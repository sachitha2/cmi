...
// 0 for BEFORE
// 1 for AFTER
$dateToConsider = htmlspecialchars($_GET["date"]);
$beforeOrAfter = htmlspecialchars($_GET["ba"]);

if($beforeOrAfter == 0){
	// Before
	$arr = $DB->select('stock','WHERE (status = 1 && exdate <= "'.$dateToConsider.'")');
}else{
	//After
	$arr = $DB->select('stock','WHERE (status = 1 && exdate >= "'.$dateToConsider.'")');
}
...
$pdf->Cell('',10,"Stock Expire Report(".$Date.')','','',"C");
...
$pdf->Cell('',6,'Days to Expire - days to expire from today','','',"L");

$main->pdfFooter($pdf);

$pdf->Output('',"Stock Expire Report(".$Date.').pdf',true);
